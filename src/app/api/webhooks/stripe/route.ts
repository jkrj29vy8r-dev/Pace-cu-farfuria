...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const { data: order, error: orderError } = await supabase.from('orders').insert({ user_email: email, stripe_payment_id: session.payment_intent as string }).select().single()
    if (orderError) {
      if (orderError.code === '23505') return NextResponse.json({ received: true })
      return NextResponse.json({ error: 'DB error' }, { status: 500 })
    }
    const token = crypto.randomBytes(32).toString('hex')
    const downloadUrl = `${process.env.NEXT_PUBLIC_URL}/api/download?token=${token}`
    await supabase.from('download_tokens').insert({ order_id: order.id, token, expires_at: new Date(Date.now() + 30*24*60*60*1000).toISOString() })
